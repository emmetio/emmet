...
define(function(require, exports, module) {
	...
	return {
		...
		splitSelector: function(selector) {
			...
			selector = this.stripComments(selector);
			var stream = stringStream(selector), ch;
			var selectors = [];

			while (ch = stream.next()) {
				if (ch == ',') {
					selectors.push(normalizeSelector(selector.substring(stream.start, stream.pos - 1)));
			...
			selectors.push(normalizeSelector(stream.current()));
