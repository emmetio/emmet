...
//				return parseItem(k, payload[k].value, payload[k].type);
		 ...
		 * with newline defined in resources
		 * @param {String} text
		 * @returns {String}
		 */
		normalizeNewline: function(text) {
			return utils.splitByLines(text).join(this.getNewline());
		}
	};
	...
	_.each(['css'], function(name) {
		...
		if (mod.resolve) {
			module.exports.addResolver(_.bind(mod.resolve, mod));
		}
	});
	...
	try {
		...
		module.exports.setVocabulary(JSON.parse(defaultSnippets), VOC_SYSTEM);
	} catch(e) {}
