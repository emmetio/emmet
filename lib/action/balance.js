...
define(function(require, exports, module) {
	...
	function rangesForCSSRule(rule, pos) {
		...
		// braces content
		var sectionValueRange = rule.valueRange(true);
		ranges.push(sectionValueRange);

		// find nested sections
		var nestedSections = cssSections.findAllRules(rule.valueRange().substring(rule.source));
		var offset = sectionValueRange.start;
		nestedSections = _.map(nestedSections, function(section) {
			return range.create2(section.start + offset, section.end + offset)
		});
