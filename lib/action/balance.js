...
define(function(require, exports, module) {
	...
	function getCSSRanges(content, pos) {
		...
		if (_.isString(content)) {
			var ruleRange = cssEditTree.matchEnclosingRule(content, pos);
			console.log('Rule range', ruleRange);
			if (ruleRange) {
	...
	function balanceCSS(editor, direction) {
		...
		if (bestMatchIx < 0 || bestMatchIx >= ranges.length) {
			...
			if (bestMatchIx >= ranges.length && direction == 'out') {
				...
				console.log('inward');
				bestMatch = cssEditTree.locateRule(content, bestMatch.start + 1);
			} else {
				bestMatch = null;
			}
		} else {
