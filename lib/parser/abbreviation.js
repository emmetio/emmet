...
//			return result;
		...
		data: function(name, value) {
			if (arguments.length == 2) {
				...
				if (name == 'resource' && elements.is(value, 'snippet')) {
					// setting snippet as matched resource: update `content`
					// property with snippet value
					var content = value.data;
					if (this._text || this.content) {
						content = abbreviationUtils.insertChildContent(content, this._text || this.content);
					}
		...
		attributeList: function() {
			...
			var res = this.matchedResource();
			if (elements.is(res, 'element') && _.isArray(res.attributes)) {
	...
	module.exports = {
		...
		expand: function(abbr, options) {
			...
			options.profile = profile.get(options.profile, options.syntax);
			tabStops.resetTabstopIndex();
			
			var data = filters.extract(abbr);
			var outputTree = this.parse(data[0], options);

			var filtersList = filters.composeList(options.syntax, options.profile, data[1]);
			filters.apply(outputTree, filtersList, options.profile);
	...
	module.exports.addOutputProcessor(function(text, node) {
		return utils.replaceCounter(text, node.counter, node.maxCount);
	});

	// XXX add tabstop updater
	module.exports.addOutputProcessor(_.bind(tabStops.abbrOutputProcessor, tabStops));

	// include default pre- and postprocessors
	_.each(['pastedContent', 'tagName', 'resourceMatcher'], function(name) {
		var mod = require('./processor/' + name);
		if (mod.preprocessor) {
			module.exports.addPreprocessor(_.bind(mod.preprocessor, mod));
		}

		if (mod.postprocessor) {
			module.exports.addPostprocessor(_.bind(mod.postprocessor, mod));
		}
	});
