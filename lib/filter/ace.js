	...
	function processTagContent(item) {
		...
		if (reOneOrTwoPipe.test(content)) {
			...
			if (pipeWithOneSpace === '| ' && reNl.test(content) === false) {
				...
				item.content = '\n' + pipeWithOneSpace.trimEnd() + '\n  ' + utils.padString(content.substring(pipeWithOneSpace.length), '  ');
			}
		} else if (reOneOrTwodot.test(content)) {
			var dotWithOneSpace = reOneOrTwodot.exec(content)[0];
			item.content = dotWithOneSpace.trimEnd() + '\n' +  utils.padString(content.substring(dotWithOneSpace.length), '')
		}	else if (reNl.test(content) && !reIndentedPipe.test(content)) {
				var pad = '  ';
				item.content = '\n||\n  ' + utils.padString(content, '  ');
			}
