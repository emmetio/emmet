/**
 * Filter for producing Ace code from abbreviation.
 */
...
define(function(require, exports, module) {
	...
	var reNl = /[\n\r]/;
	var reIndentedPipe = /^\s*(\|\n?|\|{2}\n)/;
	var reFixedDot = /^\.{1,2}\n/;
	var reOneOrTwoPipe = /^\|{1,2}\s/;
	var reOneOrTwoDot = /^\.{1,2}\s/;
	var reSpace = /^\s/;
	...
	function processTagContent(item) {
		...
		if (reOneOrTwoPipe.test(content)) {
			var pipeWithOneSpace = reOneOrTwoPipe.exec(content)[0];
			if (pipeWithOneSpace === '| ' && reNl.test(content) === false) {
				// one pipe and no newline
				item.content = '\n' + content;
			} else {
				// one pipe with newlines OR two pipes with newlines
				var pad = '  ';
				item.content = '\n' + pipeWithOneSpace.trim() + '\n  ' + utils.padString(content.substring(pipeWithOneSpace.length), pad);
			}
		} else if (reOneOrTwoDot.test(content) && !reFixedDot.test(content)) {
			// Content starts with '. ' or '.. ', neither '.\n' nor '..\n'
			// will get converted.
			// It's opinionated.
			var dotWithOneSpace = reOneOrTwoDot.exec(content)[0];
			item.content = dotWithOneSpace.trim() + '\n' + content.substring(dotWithOneSpace.length)
		}	else if (!reIndentedPipe.test(content) && !reFixedDot.test(content) && !reOneOrTwoDot.test(content) && !reOneOrTwoPipe.test(content)) {
			if (reNl.test(content)) {
				// Content has nothing to do with dot or pipe opening
				// that contains newlines will output
				// like what '|| ' starting contents output
				var pad = '  ';
