...
define(function(require, exports, module) {
	...
	function parseLinearGradient(gradient) {
		...
		// extract tokens
		/** @type StringStream */
		var stream = stringStream.create(utils.trim(gradient));
		var colorStops = [], ch;
	...
	function resolvePropertyName(name, syntax) {
		...
		if (!snippet && prefs.get('css.fuzzySearch')) {
			snippet = resources.fuzzyFindSnippet(syntax, name, 
		...
		_.each(prefs.getArray('css.gradient.prefixes'), function(prefix) {
			...
			if (prefix == 'webkit' && prefs.get('css.gradient.oldWebkit')) {
				try {
					props.push({
						name: name,
						value: module.exports.oldWebkitLinearGradient(gradient)
					});
				} catch(e) {}
			}
			...
			props.push({
				name: name,
				value: module.exports.toString(gradient, prefix)
			});
		});
	...
	function pasteGradient(property, gradient, valueRange) {
		...
		if (!valueRange) {
			valueRange = range.create(0, property.value());
		}
		...
		property.value(val(module.exports.toString(gradient)) + '${2}');
	...
	function findGradient(cssProp) {
		...
		var matchedPart = _.find(cssProp.valueParts(), function(part) {
			return gradient = module.exports.parse(part.substring(value));
		});
	...
	function expandGradientOutsideValue(editor, syntax) {
		...
		var gradient = module.exports.parse(line);
		if (gradient) {
			var props = getPropertiesForGradient(gradient, propertyName);
			props.push({
				name: propertyName,
				value: module.exports.toString(gradient) + '${2}'
			});
	...
	function findGradientFromPosition(content, pos) {
		var cssProp = null;
		/** @type EditContainer */
		var cssRule = cssEditTree.parseFromPosition(content, pos, true);
	...
	return module.exports = {
		...
		reflectValueHandler: function(property) {
			...
			_.each(property.parent.getAll(getPrefixedNames(property.name())), function(prop) {
				...
				if (m) {
					prop.value(val(that.toString(g.gradient, m[2] || '')));
				} else if ((m = prop.value().match(/\s*\-webkit\-gradient/))) {
					// old webkit gradient definition
					prop.value(val(that.oldWebkitLinearGradient(g.gradient)));
				}
			});
		...
		parse: function(gradient) {
			var result = null;
			utils.trim(gradient).replace(/^([\w\-]+)\((.+?)\)$/, function(str, type, definition) {
