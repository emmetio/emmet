...
define(function(require, exports, module) {
	...
	function getPropertiesForGradient(gradients, property) {
		...
		if (prefs.get('css.gradient.fallback') && ~propertyName.toLowerCase().indexOf('background')) {
			props.push({
				...
				value: '${1:' + gradients[0].colorStops[0].color + '}'
			});
		}
	...
	function insertGradientsIntoCSSValue(gradients, value, options) {
		...
		_.each(gradients.reverse(), function(item, i) {
			...
			value = utils.replaceSubstring(value, str + suffix, item.gradient.matchedPart);
		});
	...
	function pasteGradient(property, gradients) {
		...
		// gradient definition
		_.each(rule.getAll(cssResolver.vendorPrefixes(property.name())), function(item) {
		...
		var propsToInsert = getPropertiesForGradient(gradient, property);
	...
	return module.exports = {
		...
		findGradients: function(cssProp) {
			...
			_.each(cssProp.valueParts(), function(part) {
				if (linearGradient.isLinearGradient(part)) {
					var gradient = linearGradient.parse(part.substring(value));
					if (gradient) {
						gradients.push({
							gradient: gradient,
							matchedPart: part
		...
		reflectValueHandler: function(property) {
			var g = this.findGradients(property);
			if (!g) {
				return false;
			}
			...
			var that = this;
			_.each(property.parent.getAll(cssResolver.vendorPrefixes(property.name())), function(prop) {
				if (prop === property) {
					return;
				}

				var gradients = that.findGradients(prop);
				if (gradients) {
					var value = prop.value();
					var dfn = gradients[0].matchedPart.substring(value);
					var prefix = '';
					if (/^\s*\-([a-z]+)\-/.test(dfn)) {
						prefix = RegExp.$1;
					}
