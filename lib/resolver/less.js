...
define(function(require, exports, module) {
	...
	function resolveExtend(list) {
		...
		var hasExtend = _.find(list, function(item) {
			if (reExtend.test(item.path[item.path.length - 1])) {
				return true;
			}

			// check for `&:extend()` inside node (its children or content)
			if (item.node.content) {
				return reExtend.test(item.node.content());
			} else if (items.node.children) {
				for (var i = 0, il = items.node.children.length; i < il; i++) {
					if (reExtend.test(item.node.children[i].name())) {
						return true;
					}
				}
			}
		...
		_.each(toExtend, function(ex, i) {
			...
			var all = ex.sel !== sel;
			var options = {syntax: 'less', strict: !all};
			var targetRules = selector.rules(sel, true);

			var key = nestingKey(ex.listItem.path);
			var lookupItems = key && !all ? nestingLookup[key] : lookup;
