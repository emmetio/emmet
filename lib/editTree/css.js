...
//		return true;
	...
	var CSSEditContainer = editTree.EditContainer.extend({
		initialize: function(source, options) {
			...
			var tokens = _.isArray(source) ? source : cssParser.parse(source);
	...
	return {
		...
		findAllRules: function(content) {
			...
			try {
				while (!stream.eol()) {
					if (stream.peek() == '{') {
						matchedRanges = this._matchBracesRanges(content, stream.pos);
						
						_.each(matchedRanges, function(r) {
							var selRange = that._extractSelector(content, r.start);
		 ...
		 * @return {Range}
		 */
		_extractSelector: function(content, pos) {
			// find CSS selector
			var ch;
			var endPos = pos;
			while (--pos >= 0) {
		 ...
		 * @return {String}
		 */
		_stripComments: function(content) {
			var stream = stringStream.create(content);
			var spaces = '';
			while (!stream.eol()) {
				switch (stream.next()) {
		 ...
		 * @return {Range}
		 */
		_matchBracesRanges: function(content, pos) {
			var stream = stringStream.create(content);
