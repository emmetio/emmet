		...
		while ((token = it.next())) {
			if (token.type == 'identifier' && isValidIdentifier(it)) {
				...
				node._children.push(new CSSEditElement(node,
						editTree.createToken(propertyRange.start + offset, propertyRange.substring(source)),
						editTree.createToken(valueRange.start + offset, valueRange.substring(source)),
						editTree.createToken(end.start + offset, end.substring(source))
						));
			}
		}
	}
	...
	var CSSEditContainer = editTree.EditContainer.extend({
		initialize: function(source, options) {
			...
			_.each(topLevelRules, function(r) {
				...
				that._children.push(new CSSEditContainer(r.substring(source), {offset: r.start + options.offset}));
				sectionOffset = r.end;
			});
