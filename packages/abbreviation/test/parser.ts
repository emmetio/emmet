...
import stringify from './assets/stringify2';

describe('Parser', () => {
    function parse(abbr: string) {
        return (parser(tokenizer(abbr)));
    }

    function str(abbr: string) {
        return stringify(parse(abbr));
    }

    it.only('basic abbreviations', () => {
        console.log(str('ul.nav[title="foo"]'));
    });
