...
if (typeof (JSIO) != 'boolean') {
	...
	var FileIO = {
		...
		sfoutstreamCID : '@mozilla.org/network/safe-file-output-stream;1',
		...
		readBinary: function(file) {
			var istream = Components.classes[this.finstreamCID].
				createInstance(this.finstreamIID);

			var bstream = Components.classes[this.binstreamCID].
		...
		_read : function(dirEntry, recursive) {
			...
			try {
				while (dirEntry.hasMoreElements()) {
					list.push(dirEntry.getNext().QueryInterface(
							FileIO.localfileIID));
				}
				if (recursive) {
					var list2 = new Array();
